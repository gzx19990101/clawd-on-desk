...
**opencode** — uses the effective plugin config under `~/.config/opencode/`: `config.json` → `opencode.json` → `opencode.jsonc`, with the later file winning. Install it from **Settings → Agents** when you want local opencode tracking; after that Clawd keeps the plugin synced on launch while opencode remains enabled. You can also run `node hooks/opencode-install.js` manually. opencode **v2** (npm `@opencode/cli`) is supported on the same install: Clawd registers the v1 entry under the `plugin` config key and a v2 entry under the `plugins` key (both point into the same managed generation, so v1 and v2 never interfere). On v2 the plugin runs inside opencode's shared background service — restart the service (or start a new opencode session) after Install/Repair so the updated plugin loads, and note that "Go to terminal" focus is degraded for v2 sessions (the service owns no terminal).
...
**OMP (oh-my-pi)** — uses a per-agent extension directory: `~/.omp/agent/extensions/clawd-on-desk` in the default environment. Install it from **Settings → Agents** when you want local OMP tracking; after that Clawd keeps the extension synced on launch while OMP remains enabled. You can also run `node hooks/omp-install.js` manually, and `node hooks/omp-install.js --uninstall` to remove it. Clawd mirrors OMP's process-environment rules — `PI_CONFIG_DIR` moves the config root, `PI_CODING_AGENT_DIR` relocates the profile-less default, and `OMP_PROFILE` (falling back to `PI_PROFILE` only when undefined) selects `~/.omp/profiles/<name>/agent`; `default` or an explicitly empty `OMP_PROFILE` selects the default profile. OMP can additionally load directory overrides from home/config/agent/project `.env` files, and a project-local value varies by working directory, so run the manual installer inside that target OMP environment when it differs from Clawd's. Doctor names other on-disk default/named profiles it does not manage. Interactive OMP sessions report lifecycle and tool activity to Clawd, but OMP is state-only: Clawd shows no permission bubbles and preserves OMP's own execution behavior. If the community bridge `clawd-on-desk-omp.ts` is present in that extensions directory, Clawd skips installing its own extension (and retires only a copy it verifiably wrote earlier) so OMP never loads both and reports every event twice; Doctor reports that bridge as the intentional owner instead of offering a broken Fix action.
...
**MiniMax Code** — experimental, state-only. Clawd ships a local plugin to `~/.minimax/plugins/clawd-state/` (honors `MINIMAX_DATA_DIR` / `MAVIS_DATA_DIR`). Install it from **Settings → Agents**, or run `npm run install:minimax-hooks` / `npm run uninstall:minimax-hooks`. Covers both the `mcode` CLI and the MiniMax Code desktop app (same plugin-hook engine).
...
- **Ownership:** the whole `clawd-state/` plugin directory belongs to Clawd. Clawd proves that with an ownership marker file (`.clawd-managed.json`): a first install is built in a staging folder in the MiniMax data directory (outside `plugins/`, which MiniMax scans in full) and moved into place in one step; install refuses to touch an existing directory without a valid marker, and uninstall moves the verified directory out of `plugins/` before deleting it. A directory name or a script name inside a file never counts as ownership, so a `clawd-state/` folder left by a pre-release build (no marker) must be deleted by hand before installing. An empty `clawd-state/` counts as unclaimed and Install replaces it; a non-empty foreign directory blocks Install, and Install's error names it. If the move fails, Uninstall reports an error with the original path and keeps the install intent. If the move works but the delete fails, the moved folder has already left `plugins/`, so MiniMax does not scan it, and Uninstall succeeds as long as no Clawd plugin is back at the original location at its final check; the leftover `.clawd-minimax-removing-*` folder stays in the MiniMax data directory for you to delete by hand (Settings does not show this warning yet). Right before returning, Uninstall checks the original location once more and reports the integration as still installed if a Clawd plugin is there again (for example, one another Clawd instance just reinstalled); an install that happens after that last check is not covered. A crash mid-install or mid-uninstall can leave `.clawd-minimax-staging-*` / `.clawd-minimax-removing-*` in the data directory — MiniMax does not load those, so delete them manually; there is no automatic cleanup. Only ENOENT/ENOTDIR count as "absent": a path Clawd cannot inspect is never reported as uninstalled. `plugins/` and the data directory must share a filesystem (a cross-device rename fails clearly and is not supported). To decide whether a foreign directory still runs Clawd's hook, Clawd follows the hooks documents the manifest declares — a root `plugin.json`, `.minimax-plugin/`, or `.codex-plugin/` manifest is not modelled and reads as unconfirmable.
